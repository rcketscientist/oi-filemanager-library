...
public class ThumbnailLoader extends Thread
{
	...
	ThumbnailLoader(File file, List<IconifiedText> list, Handler handler,
			Context context, MimeTypes mimetypes)
	{
	...
	public void run()
	{
		...
		for (int x = 0; x < count; x++)
		{
			...
			try
			{
				...
				BitmapFactory.decodeFile(FileUtils.getFile(file, fileName)
						.getPath(), options);
				...
				if (options.outWidth > 0 && options.outHeight > 0)
				{
					...
					Bitmap bitmap = BitmapFactory.decodeFile(
							FileUtils.getFile(file, fileName).getPath(),
							options);
					...
					if (bitmap != null)
					{
						...
						BitmapDrawable drawable = BitmapDrawable_Compatible
								.getNewBitmapDrawable(context.getResources(),
										bitmap);

						drawable.setGravity(Gravity.CENTER);
						drawable.setBounds(0, 0, thumbnailWidth,
								thumbnailHeight);

						text.setIcon(drawable);

						Message msg = handler
								.obtainMessage(FileManagerActivity.MESSAGE_ICON_CHANGED);
						msg.obj = text;
						msg.sendToTarget();
					}
				}
